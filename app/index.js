 ...
 // Lab 2 - 2
 // for (num = 2, exp = 9, result = 2, cnt = 1; A cnt <= exp; B result *= num, cnt++) {}
 // C
 // console.log(result);
 ...
 //     for (var result = 1, cnt = 1; cnt <= n2; result *= n1, cnt++);
 //     return result;
 // }
 ...
 // var txt = 'Мы';
 // var sumLet = 2;
 // var word = s.slice(s.indexOf(txt), s.indexOf(txt) + sumLet);

 // console.log(word);

 // var s = 'Мы знаем, что монохромный цвет - это градации серого';

 // function search(txt, sumLet) {
 //     return word = s.slice(s.indexOf(txt), s.indexOf(txt) + sumLet);
 // }
 // console.log(Anword = search('цвет', 4));

 // /^\d{1,2}-\d{1,2}-\d{4}$/.test("25-02-2012") / ^ \d { 1, 2 } - \d { 1, 2 } - \d { 4 }
 // $ / .test("25-2-2012") / ^ \d { 1, 2 } - \d { 1, 2 } - \d { 4 }
 // $ / .test("01-12-2011") / ^ \d { 1, 2 } - \d { 1, 2 } - \d { 4 }
 // $ / .test("2-12-1978")

 // var s = "Smith, John\nDow, Mike\nLee, Steve";
 // s.replace(/(\w+), (\w+)/ig, "$2 $1");
 ...
 //     this.author = "Fromm";
 //     this.show = function() {
 //         console.log(this.title + " - " + this.pubYear);
 //     }
 // }

 // var book1 = new Book("Anal sex", 1999, "50$");
